...
export class GlitchUnbreach extends Action {



    recognizedCommands: string[] = ["CALM","UNBREACH","STABILIZE"];
    ...
    withTargets = (beat: AiBeat, current_room: Room, subject: Quotidian, targets: PhysicalObject[]) => {
        ...
        for (let target of targets) {
            if (target instanceof Quotidian) {
                (target as Quotidian).breached = false;
                killed = true;
            }
        }
        if (!killed) {
        ...
        return `A glitch shudders over the ${turnArrayIntoHumanSentence(targets.map((e) => e.processedName()))}, twisting their status from breached to unbreached, if it can.`;
